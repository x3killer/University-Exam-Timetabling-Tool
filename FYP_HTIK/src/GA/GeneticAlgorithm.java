...
public abstract class GeneticAlgorithm {
	...
	public abstract void evaluateChromo(Chromosome c);

	public GeneticAlgorithm(ErrorHandling problemData){
	...
	public void increasePopulationSize(int incrementCount){
		...
		this.population.sort(ChromoCompare.com);
	}
	...
	public Chromosome CrossOver(int tournamentSize){
	...
	public void insertNewChromsomesInPopulationIfTheyAreFitter(ArrayList<Chromosome> array){
		...
		while (j >= 0 && i < array.size()){
			...
			if (isCurrentChildDuplicate == false && ChromoCompare.com.compare(array.get(i), this.population.get(j)) <= 0){
	...
	//returns the best gen of the current population 
	...
	public Chromosome tournament(int tournamentSize){
		...
		for (int j = 1; j < tournamentSize; j++){
			...
			if (ChromoCompare.com.compare(nextProspectiveParent, currentBest) < 0)
	...
	public void evaluatePopulation(){
		...
		{
			this.evaluateChromo(this.population.get(i));
	...
	public void sortPopulationAccordingToFitness(){
		population.sort(ChromoCompare.com);
	}
